...
from src.models import Wav2Vec2ForSpeechClassification, HubertForSpeechClassification, BertHubertForSpeechClassification
...
class ModelArguments:
    ...
    """
    text_path: str = field(
        metadata={"help": "Path to all ASR text."}
    )
    bert_name: str = field(
        metadata={"help": "Path to Bert pretrained model"}
    )
...
def main():
    ...
    bert_tokenizer = AutoTokenizer.from_pretrained(model_args.bert_name)

    all_text = json.load(open(model_args.text_path))
    ...
    def preprocess_function(examples):
        ...
        text_list = [bert_tokenizer(all_text[path.split('/')[-1]].lower())['input_ids'][:75] for path in examples['filename']]
        text_pad_list = [[1] * len(x) + [0] * (75 - len(x)) for x in text_list]
        text_list = [x + [bert_tokenizer.pad_token_id] * (75 - len(x)) for x in text_list]

        result = feature_extractor(speech_list, sampling_rate=target_sampling_rate)
        result["labels"] = list(target_list)
        result['input_values'] = [[x, y] for x, y in zip(result['input_values'], text_list)]
        result['attention_mask'] = [[x, y] for x, y in zip(result['attention_mask'], text_pad_list)]
        # result['text_list'] = text_list
        # result['text_pad_list'] = text_pad_list
        return result

    if False:#training_args.do_train:
    ...
    pickle.dump(predict_dataset, open("test.pkl", 'wb'))
    exit()
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            with open(output_predict_file, "w", encoding="utf-8") as writer:
                ...
                writer.write("filename,emotion\n")
                for index, item in enumerate(predictions):
                    if is_regression:
                        writer.write(f"{datasets['test'][index]['filename'].split('/')[-1]},{item:3.3f}\n")
                    else:
                        item = label_list[item]
                        writer.write(f"{datasets['test'][index]['filename'].split('/')[-1]},{item}\n")
